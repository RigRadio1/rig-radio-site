...
/* FINAL OVERRIDE — PROFILE NAV AND TOP ACTIONS */
(function () {
  const NAV_ID = "profileQuickNav";
  ...
  function findButtonByText(label) {
    ...
    return buttons.find((btn) => text(btn).toLowerCase() === label.toLowerCase());
  }

  function removeTopPlayButton() {
    const editBtn = findButtonByText("Edit Profile");
    if (!editBtn || !editBtn.parentElement) return;

    const actionWrap = editBtn.parentElement;
    const siblings = Array.from(actionWrap.querySelectorAll("button, a"));

    siblings.forEach((btn) => {
      const label = text(btn).toLowerCase();
      if (btn !== editBtn && label === "play") {
        btn.remove();
      }
    });
  }

  function fixFakeFollowCounts() {
    ...
    nodes.forEach((el) => {
      ...
      if (/followers/i.test(current)) {
        el.textContent = "0 followers";
      }

      if (/following/i.test(current)) {
        el.textContent = "0 following";
      }
    });
  }

  function findSectionByHeading(words) {
    ...
    for (const heading of headings) {
      ...
      if (words.some((word) => label.includes(word))) {
        return heading.closest("section, .card, .profile-card, .songs-card, .playlist-card, .about-card, .links-card, div");
      }
    }
  ...
  function labelProfileSections() {
    const songs = findSectionByHeading(["songs", "tracks", "uploads"]);
    const playlists = findSectionByHeading(["playlist"]);
    const about = findSectionByHeading(["about"]);
    const links = findSectionByHeading(["links", "social"]);

    if (songs) songs.id = "profileSongs";
    if (playlists) playlists.id = "profilePlaylists";
    if (about) about.id = "profileAbout";
    if (links) links.id = "profileLinks";
  }
  ...
  function addProfileNav() {
    ...
    nav.innerHTML = `
      <a href="#profileSongs">Songs</a>
      <a href="#profilePlaylists">Playlists</a>
      <a href="#profileAbout">About</a>
      <a href="#profileLinks">Links</a>
    `;

    const editBtn = findButtonByText("Edit Profile");
    const profileCard =
      (editBtn && editBtn.closest(".profile-card, .member-card, .profile-header, .profile-hero")) ||
      document.querySelector(".profile-card, .member-card, .profile-header, .profile-hero, main");

    if (profileCard && profileCard.parentElement) {
      profileCard.insertAdjacentElement("afterend", nav);
      return;
    }
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", applyProfileTopFixes);
  } else {
    applyProfileTopFixes();
  }

  setTimeout(applyProfileTopFixes, 500);
  setTimeout(applyProfileTopFixes, 1500);
})();
